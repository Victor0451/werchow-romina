...
$general=$rows->rowCount();
	...
	foreach ($rows as $row) {
	$total=$total+1;
	$pesostotal=$pesostotal + $row['CUOTA'];
	}
	...
	foreach ($rows as $row) {
		$aldia=$aldia+1;
		$pesosaldia=$pesosaldia + $row['CUOTA'];
		$grupo=$row['GRUPO'];
		$contrato=$row['CONTRATO'];
		if ($grupo==1000){$rowp = Pagos::getPagos(6,$contrato);}
		else{
...
foreach ($rows as $row) {
	$pasivos=$pasivos+1;
	$pesospasivo=$pesospasivo + $row['CUOTA'];
}
$rows = SO::getSO(8,$archivo,0);
foreach ($rows as $row) {
	$activos=$activos+1;
	$pesosactivo=$pesosactivo + $row['CUOTA'];
}
