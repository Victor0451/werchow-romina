...
<body>
	...
	<div id="contenido">
		...
		<table width="100%" border=1 style="border-collapse:collapse; font-size:0.6em;">
			<thead>
				...
				<th widt="8%" bgcolor=#B2BABB>Oper.</th>
 ...
 <p><input type="button" name="btn_imp" value="Imprimir" onClick="JavaScript: window.print();" >&nbsp;<input type="button" name="btn_exp" value="Exportar" onClick="JavaScript: location.href='estad-prestamo-exp.php?estado=<?php echo $_REQUEST['estado'];?>&desde=<?php echo $_REQUEST['desde'];?>&hasta=<?php echo $_REQUEST['hasta'];?>';">&nbsp;<input type="button" name="btn_salir" value="Salir" onclick="location.href = 'prestamos_abm.php'"></p>
...
function VerPrestamos(){
	...
	if (isset($_REQUEST['btn_ver'])){
		...
		$hasta=$_REQUEST['hasta'];
		$estado=$_REQUEST['estado'];
		if ($estado=='TODOS'){$rows = Prestamo::getPrestamo2(2,$desde,$hasta,0);$bandera=1;}
		else{$rows = Prestamo::getPrestamo2(1,$desde,$hasta,$estado);}
		$ver=$rows->rowCount();
	...
	foreach ($rows as $row) {
		...
		$cont=$cont+1;
		if ($bandera==1){if ($row['ptm_estado']!='RECHAZADO'){$tot=$row['ptm_valcuota']*$row['ptm_cuotas'];
