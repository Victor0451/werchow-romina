	...
	<div id="contenido">	
		...
		<table width="100%" border=1 style="border-collapse:collapse; font-size:0.6em;">
			<thead>
				...
				<th widt="15%" bgcolor=#BB8FCE>AFILIADOS ACTIVOS</th>
				<th widt="15%" bgcolor=#BB8FCE>FACTURADO</th>
				<th widt="15%" bgcolor=#BB8FCE>COBRADO</th>
				<th widt="15%" bgcolor=#BB8FCE>MORA</th>
...
while($inicio<=$fch[1]){
	...
	ECHO $inicio.'-'.$tot_ofi.'<br>';
	if ($inicio<=9){$archivo='de_pol0'.$inicio.$fch[2];}
	else{$archivo='de_pol'.$inicio.$fch[2];}	
	
	$rows = SO::getSO(25,$archivo,0);
	$row = $rows->fetch();
	$tot_pol=$row["total"];

	if ($inicio<=9){$archivo='banco0'.$inicio.$fch[2];}
	else{$archivo='banco'.$inicio.$fch[2];}	
	
	$rows = SO::getSO(26,$archivo,0);
	$row = $rows->fetch();
	$tot_bco=$row["total"];

	if ($inicio<=9){$archivo='emi0'.$inicio.$fch[2];}
	else{$archivo='emi'.$inicio.$fch[2];}	
	...
	$facturado=$tot_cob+$tot_bco+$tot_pol+$tot_ofi+$tot_cvnio;
