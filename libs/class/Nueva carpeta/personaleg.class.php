...
//PERSONAL
class Personal {
    public function insertPersonal($legajo,$ape, $nom, $empre, $estado,$obs){
        global $db;
        //$sql="insert into personal values (NULL, '$ape', '$nom', '$esp', '$obs')";
        $sql="insert into personal (prs_legajo,prs_ape,prs_nom,prs_empre,prs_obs,prs_estado) VALUES($legajo,'$ape','$nom','$empre','$obs','$estado')";
        $db->query($sql);
    }
    ...
    public function updatePersonal($num, $legajo ,$ape, $nom, $emp, $obs, $esta){
        global $db;
        $sql="update personal set"
           ." prs_legajo=$legajo,prs_ape='$ape', prs_nom='$nom', prs_empre='$emp', prs_obs='$obs',prs_estado='$esta' "
           ." where prs_ide=$num ";
        $db->query($sql);
    }
