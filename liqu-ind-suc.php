...
<body>
	...
	<div id="contenido">
		<a href="asesores.php" class="nl"><h1>LIQUIDACION DE COMISIONES  -  <?php echo$_REQUEST['mes'].'    '.$_REQUEST['anio'];?> - SUCURSAL:&nbsp;<?php echo$_REQUEST['sucursal'];?></h1> </a>
	<form action="" id="for_cns" method="get" >
			...
			<thead>
				...
				<th widt="5%" bgcolor=#85C1E9>OF</th>
				<th widt="5%" bgcolor=#85C1E9>COB</th>
				<th widt="5%" bgcolor=#85C1E9>EST.POL</th>
...
function VerProduccion(){
    ...
    $rows=Usuario::getUsuario(17,$sucursal);            
		foreach ($rows as $row){
		$asesor= $row['usu_ide'];
		...
		$rowsP=Produccion::getProduccion2(15,$asesor,$mes,$anio,0);		
			//if($rowsP->rowCount()!=0){	
				...
				echo "<tr>
			  		...
			  		<td bgcolor=#3FF4DE><b>".$ases."</b></td>
			  		<td style='text-align:center;'bgcolor=#3FF4DE ><b>".$ventas."</td></b>
			  		<td bgcolor=#3FF4DE><b>".'$ '.$total."</td></b>
			  		...
			  		<td bgcolor=#D5DBDB style='text-align:center;' >".$tot_espol."</td>
			  	...
			  	//$totgral=$totgral+$total;
				 // echo $total;
				$total_vtas=$total+$total_vtas;
				$basico=TraerBasico($asesor);
				$ct = explode("-",$basico); 
...
//				$comision=$com1+$novell+$bas+$policia+$debito;
				$comision=$bas+$premio+$total+$bono;

				$general=$general+$comision; 
			  
			  				  	
                echo "<tr>
                <td><b>PREMIO CANT. VENTAS</b></td>
                <td  ></td>
                <td > <b>$ ".$premio."</b></td>
				 </tr>";
				 echo "<tr>
                <td><b>BONO VTAS (única vez)</b></td>
                <td  ></td>
                <td > <b>$ ".$bono."</b></td>
                 </tr>";
                 echo "<tr>
                 ...
                 <td > <b>$ ".$novell."</b></td>
			  	...
			  	<td  ></td>
			  	<td  ></td>
			  	<td bgcolor=#F4D03F><font size=4 ><b> $ ".$comision."</b></font></td>
                </tr>";
...
/*	echo "<tr>
	...
	echo "<tr>
			  	...
			  	<td bgcolor=#F4D03F style='text-align:center;' ><font size=4 color=red><b>".$ven."</b></font></td>
			  	<td bgcolor=#F4D03F style='text-align:center;' ><font size=4 color=red><b>$ ".$total_vtas."</b></font></td>
			  	<td bgcolor=#F4D03F ></td>
			  	<td bgcolor=#F4D03F ></td>
			  	<td bgcolor=#F4D03F ></td>
			  	<td bgcolor=#F4D03F ></td>
				...
				<td bgcolor=#F4D03F ></td>
				<td bgcolor=#F4D03F ></td>
			  	<td bgcolor=#F4D03F><font size=4 color=red><b> $ ".$comt."</font></b></td>
			  	</tr>";		
}	 
...
//	while ($sem<=4){*/
		$rows = Produccion::getProduccion3(3,$asesor,$smes,$sanio,0, 0);
...
/**	$sem=1;	
	 ...
	 */if  (($con>=10)and($con<20)){$basico=2000;$premio=0;$bono_unico=400;}
			
	else{ if($con>=20){$basico=4000;$premio=4400;$bono_unico=800;} else {$bono_unico=400;}//else{if($gral>=30){$basico=10000;}}	
	}

 return $basico.'-'.$premio.'-'.$bono_unico;
}

function TraerMes($mes){
$mm='';
switch ($mes) {
				case 'ENERO':$mm=1; break;
				case 'FEBRERO':$mm=2; break;
				case 'MARZO':$mm=3; break;
				case 'ABRIL':$mm=4; break;
				case 'MAYO':$mm=5; break;
				case 'JUNIO':$mm=6; break;
				case 'JULIO':$mm=7; break;
				case 'AGOSTO':$mm=8; break;
				case 'SEPTIEMBRE':$mm=9; break;
