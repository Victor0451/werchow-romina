...
<body>
	...
	<div id="contenido">
		<a href="asesores.php" class="nl"><h1>LIQUIDACION DE COMISIONES DETALLADAS  -  <?php echo$_REQUEST['mes'].'    '.$_REQUEST['anio'];?></h1> </a>
...
function VerProduccion(){
	...
	$rows=Usuario::getUsuario(7,0);
		...
		$pesos_cob=0;$pesos_of=0;$pesos_deb=0;$pesos_tjt=0;$pesos_pol=0;$pesos_conv=0;$pesos_estpol=0;
			$rowsP=Produccion::getProduccion2(9,$asesor,$mes,$anio,$estado);		
			//if($rowsP->rowCount()!=0){	
				foreach ($rowsP as $rowP) {
					$ventas=$ventas+1;
					$pp=$rowP['prod_plan'];
					
					if ($rowP['prod_plan']!='NOVELL'){$total=$total+$rowP['prod_monto'];}
					...
					switch ($rowP['prod_pago']) {
            			...
            			case 'EST POLICIA':$tot_espol=$tot_espol+1;$pesos_estpol=$pesos_estpol+150;$total=$total-$rowP['prod_monto'];break;
				...
				echo "<tr>
			  	...
			  	<td  ></td>
			  		...
			  		<td style='text-align:center;' ></td>
			  		<td style='text-align:center;' ></td>
			  		<td style='text-align:center;' ><b>$ ".$pesos_estpol."</b></td>
...
function TraerBasico($asesor){
	...
	while ($sem<=4){
		$rows = Produccion::getProduccion3(1,$asesor,$smes,$sanio, $estado, $sem);
		$con=$rows->rowCount();
		$gral=$gral+$con;
			if ($con >= 5){
				//$c2=$c2+1;
	...
	/*	else{
			$rows = Produccion::getProduccion3(0,$asesor,$smes,$sanio, $estado, $ssem);
			$con=$rows->rowCount();
	...
	/*$rowc = Sueldo::getSueldo(1,$perfil);
		foreach ($rowc as $rowcc) {
			$c1=($rowcc['sld_basico']/4)*$c2;
		}*/


	//if($gral>=20){$basico=round($bas);}
